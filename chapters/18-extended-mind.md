...
<!-- → [INFOGRAPHIC: the asymmetry between capacity-extended and direction-retained, mapped onto the phylogenetic ladder — left side: Pearl's Rung 1 capacities (gradient sensing, steering, pattern recognition, prediction) with arrows showing they appear early in the tree AND are the capacities that every tool in the catalog has extended; right side: Pearl's Rungs 2 and 3 capacities (problem formulation, counterfactual reasoning, metacognition, accountability) with arrows showing they appear late in the tree AND have never been offloaded to any tool in the catalog — student should see that the left/right split of the catalog mirrors the early/late split of the phylogenetic tree, and that this is the chapter's central non-accidental observation] -->
